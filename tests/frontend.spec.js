...
test('桌面欢迎页在固定视口对齐参考品牌、文案、流程颜色和关键几何', async ({ page }) => {
  ...
  await expect(page.locator('.brand-name')).toHaveText('教练助手');
...
test('桌面类型判定页对齐参考提示、四画像、依据和操作栏', async ({ page }) => {
  ...
  await expect(page.locator('.classification-meta')).toContainText('判断可信度');
  await expect(page.locator('.classification-meta')).toContainText('用人策略');
  await expect(page.locator('.classification-meta')).toContainText('教练模式');
