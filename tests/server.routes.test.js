...
test('提示词加载器读取对应步骤并把用户输入编码为 JSON 数据', () => {
  ...
  const planMessages = loader.buildMessages(3, payload);
  const feedbackMessages = loader.buildMessages(4, payload);
...
test('plan 与 feedback 先拒绝畸形等待态，再对合法等待态返回未就绪', async () => {
  ...
  for (const classification of malformed) {
    assert.deepEqual(await coachService.plan({
      classification, pain: '缺少能力证据。', regenerate: false, previousPlan: null,
