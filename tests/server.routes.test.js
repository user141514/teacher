...
test('默认入口把两次模型拒绝的安全诊断交给注入 logger', async () => {
  const previousApiKey = process.env.DEEPSEEK_API_KEY;
  process.env.DEEPSEEK_API_KEY = 'test-only-key';
  const warnings = [];
  const unsupported = modelClassificationResult({
    reason: '张经理指出员工在2026年8月1日导致项目进度下降50%，并且已经完成整改。',
    evidence: ['张经理指出员工在2026年8月1日导致项目进度下降50%，并且已经完成整改。'],
  });

  try {
    const service = createDefaultCoachService({
      fetchImpl: async () => okModelResponse(JSON.stringify(unsupported)),
      logger: {
        warn(event, details) {
    ...
    await assert.rejects(
      () => service.classify({ normalizedProfile: intakeResult().normalized_profile }),
      { code: 'INVALID_MODEL_RESPONSE' },
    );

    assert.equal(warnings.length, 2);
    assert.deepEqual(warnings.map(({ event, details }) => ({
      event,
    ...
    assert.doesNotMatch(JSON.stringify(warnings), /test-only-key|2026年8月1日|50%|张经理/);
  } finally {
    if (previousApiKey === undefined) {
      delete process.env.DEEPSEEK_API_KEY;
    } else {
...
function feedbackResult(overrides = {}) {
  return {
    ...
    next_steps: ['Situation（情境）：周五协作复盘。Behavior（行为）：员工在周五主动同步了风险。Impact（影响）：团队提前安排了支持。', '下周核对行动结果。'],
...
test('DeepSeek 客户端两次语义响应均无效时保持统一安全错误', async () => {
  const requestBodies = [];
  const client = createDeepSeekClient({
    apiKey: 'secret-key',
    fetchImpl: async (_url, options) => {
...
test('DeepSeek 客户端为两次语义拒绝只记录稳定诊断码', async () => {
  const warnings = [];
  let calls = 0;
  const client = createDeepSeekClient({
    apiKey: 'secret-key',
    ...
    fetchImpl: async () => {
      calls += 1;
      return okModelResponse(JSON.stringify({
        raw: calls === 1 ? 'employee private content' : 'provider private response',
      }));
    },
  });
  ...
  await assert.rejects(
    () => client.complete({
      ...
      validate: () => false,
      diagnose: (payload) => payload.raw.startsWith('employee')
        ? ['INVALID_GROW', 'employee private content']
        : ['UNSUPPORTED_RESULT', 'provider private response'],
      buildRetryMessage: (issues) => `PLAN_CONTRACT_REPAIR\n${issues.join('\n')}`,
    }),
    { code: 'INVALID_MODEL_RESPONSE' },
  );
  ...
  assert.deepEqual(warnings, [
    {
      event: 'MODEL_RESPONSE_REJECTED',
      details: { attempt: 1, issues: ['INVALID_GROW'] },
    },
    {
      event: 'MODEL_RESPONSE_REJECTED',
      details: { attempt: 2, issues: ['UNSUPPORTED_RESULT'] },
    },
  ]);
  assert.doesNotMatch(
    JSON.stringify(warnings),
    /secret-key|private user input|employee private content|provider private response/,
  );
});
...
test('四步服务按 intake、classify、plan、feedback 顺序编排并使用各步骤契约', async () => {
  ...
  const intake = await coachService.intake({
    intake: { ...intakeResult().normalized_profile },
    answers: {},
  });
  ...
  assert.deepEqual(promptLoader.calls[3].payload, {
    ...
    feedback_text: '员工在周五主动同步了风险，团队提前安排了支持。',
    requires_sbi: true,
  });
});
...
test('四步都用当前会话事实源校验模型输出并提供固定纠错消息', async () => {
  ...
  const intakeResponse = await service.intake(intakeRequest);
...
test('MODEL_FACT_BOUNDARY_REPAIR 使用稳定错误码纠正无依据事实', async () => {
  const requestBodies = [];
  let calls = 0;
  const unsupported = modelClassificationResult({
    reason: '张经理指出员工在2026年8月1日导致项目进度下降50%，并且已经完成整改。',
  ...
  const client = createDeepSeekClient({
    ...
    fetchImpl: async (_url, options) => {
      calls += 1;
      requestBodies.push(JSON.parse(options.body));
      return okModelResponse(JSON.stringify(calls === 1 ? unsupported : modelClassificationResult()));
    },
  });
  ...
  const result = await createCoachService({
    promptLoader: createTestPromptLoader(),
    client,
  }).classify({ normalizedProfile: intakeResult().normalized_profile });
...
test('事实边界两次无效时保持统一安全错误且不增加重试', async () => {
  const requestBodies = [];
  const unsupported = modelClassificationResult({
    reason: '张经理指出员工在2026年8月1日导致项目进度下降50%，并且已经完成整改。',
    evidence: ['张经理指出员工在2026年8月1日导致项目进度下降50%，并且已经完成整改。'],
  ...
  const client = createDeepSeekClient({
    ...
    fetchImpl: async (_url, options) => {
      requestBodies.push(JSON.parse(options.body));
      return okModelResponse(JSON.stringify(unsupported));
    },
  });
  ...
  assert.equal(requestBodies.length, 2);
  assert.doesNotMatch(requestBodies[1].messages.at(-1).content, /2026年8月1日|50%|张经理/);
});
...
test('feedback 仅在原始反馈非空时启用 SBI 语义校验', async () => {
  ...
  const modelResults = [feedbackResult(), factSafeFeedback];
  const coachService = createCoachService({
    promptLoader,
    client: {
      async complete({ validate }) {
  ...
  const withoutSbi = factSafeFeedback;
